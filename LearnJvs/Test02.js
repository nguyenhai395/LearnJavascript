// console.log(`Well Come to training with JVScript `)
// console.log(`For Lesson 8`)
// let a= 1
// let b=2.2
// let c=3
// console.log(` For training part 7`)
// console.log(` Seven is ${a+(c*2)} is not ${a+b}`)
// console.log(`--------------------`)
// let inf={
//     name: "Hary",
//     address: "VietNam",
//     email : "nguyenhhai86@gmail.com"
// }

// console.log(`My name is `, inf.name)
// console.log(`My address is `,inf.address)
// console.log(`My email is `, inf.email)

// console.log(` Part 9 Array `)
// let x =["MU","Liver", "Chealsea"]
// console.log(`type of x`, typeof x, x)
// console.log(`First item IS `, x[0])
// console.log(`Second item IS `, x[1])

// BAT DAU VOI PART 10 null empty not difined
// let c ;
// console.log(` Value of C `, c)

//Start with Lesson 11
let array= [" MU","Mancity","chealsea","Liver","RealMadr"]
for(let i=0;i<array.length;i++)
{
    console.log("Cac doi tuyen la ",array[i]);
}